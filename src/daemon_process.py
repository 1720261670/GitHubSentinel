...
def github_job(subscription_manager, github_client, report_generator, notifier, days):
    ...
    LOG.info(f"[定时任务执行github完毕]")

def hacker_news_job(hacker_news_client, hacker_news_report_generator, notifier):
    LOG.info("[开始执行hackernews定时任务]")
    # 获取当前时间热点信息
    markdown_file_path, today = hacker_news_client.export_daily_progress()
    # 从Markdown文件自动生成进展简报
    report, report_file_path = hacker_news_report_generator.generate_daily_report(markdown_file_path)
    notifier.hacker_news_notify(today, report)
    LOG.info(f"[定时任务执行hackernews完毕]")
...
def main():
    ...
    llm = LLM("report_prompt.txt")  # 创建语言模型实例
    ...
    llm = LLM("hacker_news_prompt.txt")  # 创建语言模型实例
    hacker_news_report_generator = HackerNewsReportGenerator(llm)
    hacker_news_job(hacker_news_client, hacker_news_report_generator, notifier)
    # 安排每天的定时任务
    schedule.every(config.hacker_news_freq_days).days.at(
        config.hacker_news_exec_time
    ).do(hacker_news_job, hacker_news_client, hacker_news_report_generator, notifier)
