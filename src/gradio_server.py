...
def generate_wb_hour_topic(model_type, model_name):
    config.llm_model_type = model_type
    ...
    report, report_file_path = report_generator.generate_wb_topic_report(markdown_file_path)
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("微博 热搜"):
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_wb_hour_topic, inputs=[model_type, model_name,], outputs=[markdown_output, file_output])
